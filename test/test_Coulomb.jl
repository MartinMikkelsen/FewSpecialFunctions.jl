...
@testset "CoulombF_imag" begin
        ...
        for r in 1:size(data, 1)
            ...
            η        = row[2]
            ρ        = row[3]
        @test real.(FewSpecialFunctions.F(ℓ,η,ρ)) ≈ real.(FewSpecialFunctions.F_imag(ℓ,η,ρ)) atol=1e-7
...
@testset "η" begin
    ...
    @test η(4.0) ≈ 1/2.0 atol=1e-10
end

@testset "C normalization" begin
    # for η=0, ℓ=0: C = 1
    @test C(0, 0.0) ≈ 1.0 atol=1e-10
    # for η=0, ℓ=1: C = 2^1 * |Γ(2)/Γ(4)| = 2 * (1!)/(3!) = 2/6 = 1/3
    @test C(1, 0.0) ≈ 1//3 atol=1e-10
end
...
@testset "F regular Coulomb function" begin
    ...
    for ρ in (0.1, 1.0, 2.5, π/3)
        @test real(F(0,0.0,ρ)) ≈ sin(ρ) atol=1e-8
        @test abs(imag(F(0,0.0,ρ))) ≤ 1e-8
    end
end

@testset "D⁺ and D⁻ symmetry" begin
    for (ℓ, η) in ((0, 0.5), (1, 1.2), (2, -0.7))
        @test conj(D⁺(ℓ,η)) ≈ D⁻(ℓ,η) atol=1e-10
    end
end

@testset "H⁺ and H⁻ symmetry" begin
    for (ℓ, η, ρ) in ((0,0.5,1.0), (1,1.0,2.0), (1.5,-0.3,3.5))
        @test conj(H⁺(ℓ,η,ρ)) ≈ H⁻(ℓ,η,ρ) atol=1e-10
    end
end

@testset "F_imag and G combination" begin
    for (ℓ, η, ρ) in ((0,0.5,1.0), (1,1.0,2.0), (1.5,-0.7,3.5))
        @test F_imag(ℓ,η,ρ) ≈ F(ℓ,η,ρ) atol=1e-10
        @test G(ℓ,η,ρ) ≈ real(G(ℓ,η,ρ)) atol=1e-10  # G should be real
    end
end

@testset "M_regularized" begin
    α, β, γ = 2.0, 3.0, 4.0
    @test M_regularized(α,β,γ) ≈ HypergeometricFunctions._₁F₁(α,β,γ)/gamma(β) atol=1e-10
end
...
@testset "Gamow factor w" begin
    ...
    # half-integer ℓ=1/2
    @test w(0.5, 2.0) ≈ (1 + 0.5^2/2.0^2) atol=1e-10
end
