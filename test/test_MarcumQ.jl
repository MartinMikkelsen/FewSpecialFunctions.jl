...
@testset "Marcum Q-function" begin
    ...
    @testset "MarcumQ vs MATLAB reference" begin
        ...
        for r in 1:size(data, 1)
            ...
            @test FewSpecialFunctions.MarcumQ(M, a, b) ≈ Q_ref rtol = 1.0e-10
        end
    end


    # Acta Univ. Sapientiae, Mathematica, 3, 1 (2011) 60–7
    @test FewSpecialFunctions.MarcumQ(1, 0.2, 0.6) ≈ 0.838249985438908 atol = 1.0e-9
    @test FewSpecialFunctions.MarcumQ(5, 0.2, 0.6) ≈ 0.999998670306184 atol = 1.0e-9
    @test FewSpecialFunctions.MarcumQ(7.7, 0.2, 0.6) ≈ 0.999999999927717 atol = 1.0e-9

    @test FewSpecialFunctions.MarcumQ(1, 1.2, 1.6) ≈ 0.501536568390858 atol = 1.0e-9
    @test FewSpecialFunctions.MarcumQ(5, 1.2, 1.6) ≈ 0.994346394491553 atol = 1.0e-9
    @test FewSpecialFunctions.MarcumQ(7.7, 1.2, 1.6) ≈ 0.99994493722354 atol = 1.0e-9

    @test FewSpecialFunctions.MarcumQ(1, 2.2, 2.6) ≈ 0.426794627821735 atol = 1.0e-9
    @test FewSpecialFunctions.MarcumQ(5, 2.2, 2.6) ≈ 0.929671935077756  atol = 1.0e-9
    @test FewSpecialFunctions.MarcumQ(7.7, 2.2, 2.6) ≈ 0.993735633182201 atol = 1.0e-9
    ...
    @test FewSpecialFunctions.MarcumQ(5, 0.47, 4.85) ≈ 0.0106766402997493 atol = 1.0e-9
    @test FewSpecialFunctions.MarcumQ(5, 1.46, 4.0) ≈ 0.211798804811782  atol = 1.0e-9
    @test FewSpecialFunctions.MarcumQ(1, 1.27, 4.58) ≈ 0.000931257801666407 atol = 1.0e-9
    @test FewSpecialFunctions.MarcumQ(4, 2.88, 3.28) ≈ 0.773155207859263 atol = 1.0e-9
    @test FewSpecialFunctions.MarcumQ(1, 2.55, 4.67) ≈ 0.024112315799424 atol = 1.0e-9
    @test FewSpecialFunctions.MarcumQ(4, 2.27, 3.72) ≈ 0.400088995953665 atol = 1.0e-9
    @test FewSpecialFunctions.MarcumQ(2, 1.97, 0.86) ≈ 0.990345203236692 atol = 1.0e-9
    @test FewSpecialFunctions.MarcumQ(4, 0.1, 1.38) ≈ 0.983869651076909 atol = 1.0e-9
    @test FewSpecialFunctions.MarcumQ(1, 0.29, 4.12) ≈ 0.00028475422733874 atol = 1.0e-9
    @test FewSpecialFunctions.MarcumQ(4, 0.95, 4.75) ≈ 0.00899422673877906 atol = 1.0e-9
...
@testset "MarcumQ overloads" begin
    ...
    @testset "Array inputs" begin
        ...
        @test q_b_arr[1] ≈ FewSpecialFunctions.MarcumQ(1, 0.2, 0.6) atol = 1.0e-9
        @test q_b_arr[2] ≈ FewSpecialFunctions.MarcumQ(1, 0.2, 1.6) atol = 1.0e-9
        @test q_b_arr[3] ≈ FewSpecialFunctions.MarcumQ(1, 0.2, 2.6) atol = 1.0e-9
        ...
        @test q_a_arr[1] ≈ FewSpecialFunctions.MarcumQ(1, 0.2, 0.6) atol = 1.0e-9
        @test q_a_arr[2] ≈ FewSpecialFunctions.MarcumQ(1, 1.2, 0.6) atol = 1.0e-9
        @test q_a_arr[3] ≈ FewSpecialFunctions.MarcumQ(1, 2.2, 0.6) atol = 1.0e-9
        ...
        @test q_μ_arr[1] ≈ FewSpecialFunctions.MarcumQ(1.0, 0.2, 0.6) atol = 1.0e-9
        @test q_μ_arr[2] ≈ FewSpecialFunctions.MarcumQ(5.0, 0.2, 0.6) atol = 1.0e-9
        @test q_μ_arr[3] ≈ FewSpecialFunctions.MarcumQ(7.7, 0.2, 0.6) atol = 1.0e-9
    end

    # Test for the default μ=1 case
    @testset "Default μ=1" begin
        @test FewSpecialFunctions.MarcumQ(0.2, 0.6) ≈ FewSpecialFunctions.MarcumQ(1, 0.2, 0.6) atol = 1.0e-9
        @test FewSpecialFunctions.MarcumQ(1.2, 1.6) ≈ FewSpecialFunctions.MarcumQ(1, 1.2, 1.6) atol = 1.0e-9
        @test FewSpecialFunctions.MarcumQ(2.2, 2.6) ≈ FewSpecialFunctions.MarcumQ(1, 2.2, 2.6) atol = 1.0e-9
    end
end
...
@testset "dQdb - Marcum Q derivative" begin
    ...
    function numerical_dQdb(M, a, b, h = 1.0e-6)
    ...
    for (M, a, b) in test_cases
        ...
        @test analytical ≈ numerical rtol = 1.0e-4
    end

    # Edge cases
    @test FewSpecialFunctions.dQdb(1, 0.1, 0.1) ≈ numerical_dQdb(1, 0.1, 0.1) rtol = 1.0e-4  # Small values
    @test FewSpecialFunctions.dQdb(10, 5.0, 5.0) ≈ numerical_dQdb(10, 5.0, 5.0) rtol = 1.0e-3  # Larger order
...
@testset "Helper functions for MarcumQ" begin
    @testset "Q function (regularized incomplete gamma)" begin
        ...
        @test FewSpecialFunctions.Q(1.0, 0.0) ≈ 1.0 atol = 1.0e-14
        @test FewSpecialFunctions.Q(1.0, 1.0) ≈ exp(-1.0) atol = 1.0e-14
        @test FewSpecialFunctions.Q(2.0, 2.0) ≈ exp(-2.0) * (1.0 + 2.0) atol = 1.0e-14
        @test FewSpecialFunctions.Q(3.0, 3.0) ≈ exp(-3.0) * (1.0 + 3.0 + 9.0 / 2.0) atol = 1.0e-14

        # Compare with direct computation for large values
        μ, x = 5.0, 10.0
        @test FewSpecialFunctions.Q(μ, x) ≈ gamma_inc(μ, x)[2] atol = 1.0e-14
    end
    ...
    @testset "c_μ function (Bessel I ratio)" begin
        ...
        for μ in [0.5, 1.0, 1.5, 2.0]
            for ξ in [0.1, 0.5, 1.0, 2.0, 5.0]
                ...
                @test FewSpecialFunctions.c_μ(μ, ξ) ≈ direct_ratio atol = 1.0e-14
            end
        end
        ...
        for μ in [1.0, 2.0, 3.0]
            ...
            @test FewSpecialFunctions.c_μ(μ, large_ξ) ≈ approx rtol = 1.0e-3
        end
    end
end
...
@testset "FewSpecialFunctions dQdb Tests" begin
    ...
    @testset "dQdb Scalar Tests" begin
        ...
        result = dQdb(M, a, b)
        @test isapprox(result, expected, atol = 1.0e-6)

        # Convenience method: M=1
        a, b = 2.0, 3.0
        result = dQdb(a, b)
        @test isapprox(result, dQdb(1, a, b), atol = 1.0e-6)
    end
    ...
    @testset "dQdb Array Tests" begin
        ...
        @test isapprox(results[2], expected2, atol = 1.0e-6)
        ...
        expected2 = -3.0 * exp(-6.5) * besseli(0, 6.0)
        @test isapprox(results[1], expected1, atol = 1.0e-6)
        @test isapprox(results[2], expected2, atol = 1.0e-6)
...
@testset "Q (upper incomplete gamma)" begin
    ...
    @test Q(1.0, 2.0) ≈ exp(-2.0) atol = 1.0e-9
    # Γ(2,x) = e^{-x}(x+1)
    @test Q(2.0, 3.0) ≈ exp(-3.0) * (3.0 + 1.0) atol = 1.0e-9
    # at x=0, Q(μ,0) == Γ(μ)
    μ = 3.5
    @test Q(μ, 0.0) ≈ 1.0 atol = 1.0e-9
    @test Q(μ, 100.0) ≈ 0.0 atol = 1.0e-9
end

@testset "c_μ (continued‐fraction ratio for scaled I_μ)" begin
    for μ in (0.0, 0.5, 1.0, 2.5)
        for ξ in (0.1, 1.0, 5.0)
            @test c_μ(μ, ξ) ≈ besseli(μ, ξ) / besseli(μ - 1, ξ) atol = 1.0e-9
        end
    end
end
...
@testset "half_ζ2 (ζ²/2 from Eq. 84)" begin
    ...
    for x in (0.5, 1.0, 2.0)
        y = x + 1.0
        @test half_ζ2(x, y) ≈ 0.0 atol = 1.0e-9
    end
    # general branch
    x, y = 1.2, 3.4
    δ = y - x - 1.0
    ...
    @test half_ζ2(x, y) ≈ expected atol = 1.0e-9
end


@testset "ζ (Eq. 56)" begin
    @test ζ(0.5, 1.5) ≈ copysign(sqrt(2 * half_ζ2(0.5, 1.5)), 0.5 + 1.0 - 1.5) atol = 1.0e-9
end

@testset "θ/sin(θ) and θ′sin(θ) for small θ" begin
    @test theta_over_sin(0.0) ≈ 1.0 atol = 1.0e-9
    @test theta_over_sin(1.0e-6) ≈ 1.0 + (1.0e-6)^2 / 6 atol = 1.0e-9
    @test theta_over_sin(0.1) ≈ 0.1 / sin(0.1) atol = 1.0e-9
    @test theta_prime_sin(0.0) ≈ 0.0 atol = 1.0e-9
    @test theta_prime_sin(1.0e-6) ≈ (1.0e-6)^2 / 3 atol = 1.0e-9
    @test theta_prime_sin(0.1) ≈ 1.0 - 0.1 / tan(0.1) atol = 1.0e-9
end

@testset "ρ from eq. (98)" begin
    @test ρ(0.0, 1.0) ≈ 1.0 atol = 1.0e-9
    @test ρ(1.0, 0.0) ≈ 1.0 atol = 1.0e-9
    @test ρ(1.0, 1.0) ≈ sqrt(2.0) atol = 1.0e-9
end
...
@testset "r and r′sin(θ) from eq. (96)" begin
    ...
    @test r(θ, y, ξ) ≈ expected_r atol = 1.0e-9
    expected_r_prime = (1.0 + 1.0 / sqrt(1.0 + xs^2)) * theta_prime_sin(θ) / (2.0 * y)
    @test r_prime_sin(θ, y, ξ) ≈ expected_r_prime atol = 1.0e-9
end
...
@testset "f from eq. (96)" begin
    ...
    @test f(θ, y, ξ) ≈ expected_f atol = 1.0e-9
end
...
@testset "ψ from eq. (97)" begin
    ...
    @test ψ(θ, ξ) ≈ expected_ψ atol = 1.0e-9
end

@testset "f1_f2 (boundaries for recurrence relation)" begin
    x, M = 1.0, 2.0
    f1, f2 = f1_f2(x, M)
    @test f1 ≈ x + M - sqrt(4 * x + 2 * M) atol = 1.0e-9
    @test f2 ≈ x + M + sqrt(4 * x + 2 * M) atol = 1.0e-9
    ...
    for (x, M) in test_cases
        ...
        @test f1 ≈ expected_f1 atol = 1.0e-9
        @test f2 ≈ expected_f2 atol = 1.0e-9

        @test f1 < f2

        @test f2 - f1 ≈ 2 * sqrt(4 * x + 2 * M) atol = 1.0e-9
    end
    ...
    @test f1 ≈ x + M - sqrt(4 * x + 2 * M) atol = 1.0e-6
    @test f2 ≈ x + M + sqrt(4 * x + 2 * M) atol = 1.0e-6
end
...
@testset "MarcumQ_quadrature (numerical quadrature evaluation)" begin
    ...
    @test isapprox(Qq, Qm; atol = 1.0e-7)  # Allow some tolerance due to numerical integration
...
@testset "MarcumQ_recurrence (recurrence relation for Marcum Q)" begin
    ...
    Qm = FewSpecialFunctions.MarcumQ_modified(M, x, y)
    @test isapprox(Qr, Qm; atol = 1.0e-6)
    ...
    if abs(M - μ) < 1.0e-12
        Qr = FewSpecialFunctions.MarcumQ_recurrence(M, x, y, ξ)
        Qm = FewSpecialFunctions.MarcumQ_modified(M, x, y)
        @test isapprox(Qr, Qm; atol = 1.0e-10)
    end
end
...
@testset "MarcumQ(M::T, a::T, b::T) basic correctness" begin
    ...
    a = 1.5
    for b1 in 0.5:0.5:2.0, b2 in 0.5:0.5:2.0
        if b1 < b2
            @test FewSpecialFunctions.MarcumQ(1.0, a, b1) ≥ FewSpecialFunctions.MarcumQ(1.0, a, b2)
        end
    end
