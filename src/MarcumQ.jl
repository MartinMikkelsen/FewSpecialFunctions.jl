...
theta_over_sin(θ::T) where {T <: Number} = θ < T(1.0e-4) ? one(T) + θ^2 / T(6) : θ / sin(θ)
theta_prime_sin(θ::T) where {T <: Number} = θ < T(1.0e-4) ? θ^2 * (θ^2 / T(45) + one(T) / T(3)) : one(T) - θ / tan(θ)
...
function r(θ::T, y::T, ξ::T) where {T <: Number}
    ...
    return (one(T) + sqrt(one(T) + xs^2)) * tos / (T(2) * y)
end

function r_prime_sin(θ::T, y::T, ξ::T) where {T <: Number}
    tos = theta_over_sin(θ); xs = ξ / tos
    return (one(T) + one(T) / sqrt(one(T) + xs^2)) * theta_prime_sin(θ) / (T(2) * y)
end
