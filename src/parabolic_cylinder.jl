...
function W(a::Float64, x::Float64)::Float64
    ...
    if abs(x) ≤ 5
        ...
        g0 = gamma(Complex(1/2, a))
        φ₂ = imag(g0)

        gref = gamma(Complex(1/2, a))
        gr0, gi0 = real(gref), imag(gref)
        den = gr0^2 + gi0^2
        ...
        for k in 2:2:40
            ...
            gr, gi = real(g), imag(g)
            u[m] = (gr * gr0 + gi * gi0) / den
            v[m] = (gr0 * gi - gr * gi0) / den
        end
...
using Plots 
xs = -10:0.1:10
plot(xs,dU(0.5, xs), label="U(0.5, x)")